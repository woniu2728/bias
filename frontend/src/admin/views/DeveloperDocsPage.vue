...
const guides = [
  {
    ...
    title: '模块开发指南',
    description: '说明模块定义、资源注册和前后端入口的接入方式。',
    filename: 'module-development.md',
    content: '# 模块开发指南\n\n1. 先确定模块边界，再注册后端扩展、前端入口和资源定义。\n2. 后端能力优先挂到资源、服务和生命周期钩子上，减少页面侧直连。\n3. 前端页面只消费已注册的运行时能力，不直接假设接口存在。',
  },
  {
    key: 'resource-fields',
    title: 'Resource 字段扩展指南',
    description: '说明 Resource 字段如何注册、组合与消费。',
    filename: 'resource-fields.md',
    content: '# Resource 字段扩展指南\n\n1. 字段注册应跟随资源语义，不要把展示逻辑塞进字段 resolver。\n2. 列表、详情和搜索结果可以共享 resolver，但注意预加载与性能。\n3. 对外输出要保持字段稳定，避免前端按临时结构写死。',
  },
  {
    key: 'event-subscriptions',
    title: '事件订阅指南',
    description: '说明领域事件与监听器接入方式。',
    filename: 'event-subscriptions.md',
    content: '# 事件订阅指南\n\n1. 事件订阅只处理领域动作，不做页面跳转或 UI 副作用。\n2. 订阅逻辑要幂等，失败后不能污染主流程。\n3. 对高频事件优先做合并、去重和异步化处理。',
  },
  {
    key: 'frontend-injection-points',
    title: '前端注入点指南',
    description: '说明 header、动作、状态块等扩展入口。',
    filename: 'frontend-injection-points.md',
    content: '# 前端注入点指南\n\n1. 注入点应保持局部职责，不要替代整页路由。\n2. 组件需要兼容无登录、加载中和权限不足三种基础状态。\n3. 交互要和宿主应用的视觉与布局约定保持一致。',
  },
  {
    key: 'permission-registration',
    title: '权限注册指南',
    description: '说明权限码、别名和依赖关系的注册方式。',
    filename: 'permission-registration.md',
    content: '# 权限注册指南\n\n1. 权限码必须全局唯一，命名要稳定。\n2. 后端校验和前端展示应使用同一套权限语义。\n3. 新权限应补齐默认角色、迁移和回归测试。',
  },
  {
    key: 'admin-page-registration',
    title: '后台页面注册指南',
    description: '说明后台页面如何同时接入后端模块定义与前端路由。',
    filename: 'admin-page-registration.md',
    content: '# 后台页面注册指南\n\n1. 后台页面先注册模块，再挂前端路由和权限控制。\n2. 页面入口需要在未登录和权限不足时给出明确退路。\n3. 列表页、详情页和编辑页应共享统一的数据源。',
  },
  {
    key: 'platform-delivery',
    title: '平台交付与性能收尾指南',
    description: '说明部署、缓存、性能和上线前检查项。',
    filename: 'platform-delivery.md',
