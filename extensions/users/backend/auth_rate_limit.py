...
def clear_auth_rate_limit(action: str, request, identifier: str = "") -> None:
    cache.delete_many(_rate_limit_keys(action, request, identifier))


def _rate_limit_keys(action: str, request, identifier: str = "") -> list[str]:
    ip = _client_ip(request)
    normalized_identifier = str(identifier or "").strip().lower()
    keys = [f"auth-rate-limit:{action}:ip:{_digest(ip)}"]
