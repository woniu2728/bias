...
首次部署使用本地源码构建 wheel，站点目录应与核心包、基础内容包和官方扩展位于同一个工作区：
...
安装脚本会检查 `bias_core`、`bias-content` 和至少一个 `bias-ext-*` 是否存在。
...
官方扩展位于 `bias` 同级目录，名称为 `bias-ext-*`。`bias-content` 是基础内容包，也必须位于 `bias` 同级目录。安装或升级脚本会自动构建 `bias_core`、`bias-content` 和这些扩展的 wheel，并在容器镜像中安装。
