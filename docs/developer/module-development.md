...
3. 使用稳定的扩展 id 作为所有扩展能力的归属标识。
4. 只注册当前切片需要的能力，避免一次性过度铺开。
5. 需要前台扩展时，在扩展自己的 `frontend/forum/index.js` 或 `frontend/admin/index.js` 中挂接注入点。
6. 补扩展中心可见信息、开发者文档和测试，保证注册结果可见、可检验。
...
1. 后端注册
   在 `extension.json` 中声明扩展、依赖、兼容性、分发信息和生命周期操作。
2. 管理入口
