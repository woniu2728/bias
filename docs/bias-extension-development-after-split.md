...
- 站点前端构建前，通过 `bias/frontend/scripts/syncExtensionSources.mjs` 同步到 `bias/extensions/<id>`。
- `bias/extensions/<id>` 下的官方扩展目录是生成的构建输入，不是源码目录。
- `@bias/core/*` 目前仍由 `bias/frontend` 内的 alias 提供，尚未拆成独立 npm 包。
