...
| 安装/更新 | `src/Install`、`src/Update` | `bias_core.management.commands.install_forum`、`upgrade_forum`、`migrate_extensions`、`sync_extensions`、`bias` project bootstrap | partial | `bias_core/src/bias_core/management/commands/install_forum.py`、`upgrade_forum.py`、`migrate_extensions.py`、`sync_extensions.py`；`bias_core/tests/test_bootstrap_config.py`、`test_extension_service.py` 有部分覆盖；`python manage.py smoke_install_upgrade --from-wheels --skip-collectstatic --format json` 已构建 `bias-core`、`bias-content` 和 16 个官方 `bias-ext-*` wheel，安装到临时 `site-packages` target，并在临时 SQLite 站点执行安装和升级，验证管理员、已安装扩展、已启用扩展、`system.version` 和 `bias_core.__file__` 来自 wheel target；`upgrade_forum` 已支持拆分站点无 `VERSION` 文件时使用 `bias_core.APP_VERSION` 校验 | 缺 PostgreSQL 安装冒烟，以及发布级前端 build/collectstatic 全链路验证。 |
