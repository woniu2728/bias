...
| --- | --- | --- | --- | --- | --- |
| 用户模型 | `src/User` | `bias-ext-users` | partial | `bias_ext_users/backend/models.py`、`services.py`、`api.py`、`admin_api.py`；后台用户/组/权限 API 已有核心测试：`bias_core/tests/test_admin_permissions.py`、`test_management.py`；`bias_ext_users/backend/tests.py` 覆盖 `search_user` 资源支持 `include=groups` 并保留 `primary_group` 字段；`bias/frontend/e2e/forum-home.spec.js` 覆盖真实 Chromium 下 `/profile` 加载 `/api/users/me`、用户讨论、用户回复、偏好，以及设置页 `PATCH /api/users/{id}` 保存资料，并覆盖公开 `/u/{id}` 用户资料页；同一 E2E 还覆盖未登录直达受保护创建页后登录并回到 composer、注册、忘记密码 debug reset link、邮箱验证和重置密码页面；`bias-ext-users/frontend/forum/useAuthRoutePage.test.js` 覆盖 auth route modal 成功认证后保留 redirect | 继续补头像上传、登录/注册失败路径、人机验证、邮箱重发和更多账号安全 HTTP 集成矩阵。 |
...
1. 扩展 discussion/post 的 HTTP 集成覆盖。
   ...
   - 下一步继续收敛通知触发、删除、筛选刷新，以及头像上传、账号安全失败路径等用户相关 HTTP 与浏览器矩阵。
