...
1. 扩展 discussion/post 的 HTTP 集成覆盖。
   ...
   - 下一步继续收敛真实内容操作触发通知、通知邮件/队列投递，以及头像上传、账号安全失败路径等用户相关 HTTP 与浏览器矩阵。
