...
| 通知 | `src/Notification` | `bias-ext-notifications` | partial | `bias_ext_notifications/backend/services.py`、`resources.py`、`tasks.py`、`listeners.py` 存在；runtime service contract 已在 workspace gate 中检查；`bias-ext-notifications/bias_ext_notifications/backend/tests.py` 已覆盖 type count、`read-filtered`、`clear-filtered-read`、单条 soft delete 和 reply/mention/like/approval 触发矩阵，并覆盖 `NotificationCreatedEvent` 监听后派发 `dispatch_notification_batch`、Celery 任务投递通知邮件、队列入队失败同步回退继续加载实时通知和发送邮件；`bias/frontend/e2e/forum-home.spec.js` 覆盖真实 Chromium 下 `/notifications` 请求 `/api/notifications`、渲染 reply/account 通知、单条 `POST /api/notifications/{id}/read` 标记已读、确认后 `DELETE /api/notifications/read/clear` 清理已读并进入空状态，并覆盖 `type`/`state=unread` 筛选刷新、`POST /api/notifications/read-filtered`、单条 `DELETE /api/notifications/{id}` 删除和 `DELETE /api/notifications/read/clear-filtered`；同一 E2E 已覆盖用户在讨论详情提交 `POST /api/discussions/{id}/posts` 后生成新的 `postReply` 通知，再进入 `/notifications` 渲染新通知和未读计数 | 继续补 flag 组合触发，以及真实 Redis worker/生产冒烟证据。 |
...
1. 扩展 discussion/post 的 HTTP 集成覆盖。
   ...
   - 下一步继续补真实 Redis worker/生产冒烟、flag 组合触发和后台浏览器流程。
