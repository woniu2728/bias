...
| 安装/更新 | `src/Install`、`src/Update` | `bias_core.management.commands.install_forum`、`upgrade_forum`、`migrate_extensions`、`sync_extensions`、`bias` project bootstrap | partial | `bias_core/src/bias_core/management/commands/install_forum.py`、`upgrade_forum.py`、`migrate_extensions.py`、`sync_extensions.py`；`bias_core/tests/test_bootstrap_config.py`、`test_extension_service.py` 有部分覆盖；`python manage.py smoke_install_upgrade --from-wheels --publish-frontend-dist --format json` 已构建 `bias-core`、`bias-content` 和 16 个官方 `bias-ext-*` wheel，安装到临时 `site-packages` target，并在临时 SQLite 站点执行安装和升级，验证管理员、已安装扩展、已启用扩展、`system.version` 和 `bias_core.__file__` 来自 wheel target；安装/升级均执行 `build_extension_frontend --rebuild --publish` 与 `collectstatic --noinput`，断言临时 `staticfiles/extensions` 下的前端 manifest 存在且 `staticfiles/frontend` 收集到 129 个前端构建文件；`python manage.py smoke_install_upgrade --database postgres --postgres-create-db --postgres-drop-db ... --format json` 已在一次性 PostgreSQL 数据库上执行安装和升级，验证管理员、17 个官方启用扩展、`system.version`、`advanced.queue_enabled=true`、`advanced.queue_driver="redis"` 保持，并删除临时库；`install_forum` 的安装运行时设置已改为通过目标站点配置子进程写入；`upgrade_forum` 已支持拆分站点无 `VERSION` 文件时使用 `bias_core.APP_VERSION` 校验 | 阶段 8 主缺口已收敛；继续进入阶段 9 全量测试门槛和阶段 10 浏览器 E2E 门槛。 |
| 扩展管理 | `src/Extension`、`src/Extend` | `bias_core.extensions`、admin extensions API、frontend extension pages | partial | `bias_core/tests/test_admin_extensions_api.py`、`test_extension_service.py`、`test_extension_registry.py`、`test_extension_boundary.py`、`check_extension_workspace` gate 覆盖大量后端能力；`npm run check:platform` 覆盖 17 个官方扩展前端 source tree 同步、SDK package 和前端边界；`bias/frontend/e2e/admin-extensions.spec.js` 覆盖扩展中心浏览器闭环：列表、启停、protected 约束、sync/order/rebuild、diagnostics/recovery/frontend asset 状态；`bias_core/tests/test_extension_commands.py` 覆盖 `create_extension --target`、生成扩展 wheel 构建、manifest/backend entry point/frontend/locale 资源入包、安装态 discovery，以及干净安装态 `ExtensionManager` install/disable/enable 生命周期 smoke | 可继续补干净站点安装态后台 UI 对真实后端的浏览器流程；当前主要缺口转向阶段 8 的非 editable 全新安装/升级冒烟和阶段 10 的全链路测试。 |
