...
1. 扩展 discussion/post 的 HTTP 集成覆盖。
   ...
   - 下一步继续补非 staff moderation 权限一致性、rejected 后重新提交、SQL 预算和浏览器流程。
